...
test("記録の時刻は持ち主の時計で見せる(夜中の記録を前日にしない)", () => {
  // 実際に踏んだ形。2026-08-11 00:52 JST に書いた行は台帳では 2026-08-10T15:52Z。
  // これを帯なしで渡すと、モデルには別口で「今日は 2026年8月11日」と入っているので、
  // 51分前の出来事を昨日の午後として読む。
  ...
  // 読めない値は握り潰さずそのまま返す。台帳の古い行を落とさないため。
