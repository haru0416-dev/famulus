...
test("日本語が入っていない語は、絞らず1回だけ引く(枠を倍に使わない)", async () => {
  // 実測(2026-08-11): 英語の識別子で引く回は `stocks:>10` が 25回中 21回 0件を返し、
...
test("回数制限に当たった先は、解けるまで叩かない", async () => {
  // 実測(2026-08-11): 端から端まで1回動かしただけで Qiita の無認証枠 60回/時を使い切り、
...
test("x — from:名前 と @名前 は、その人の投稿だけに絞る", async () => {
  // 実測(2026-08-11): `site:x.com/youyuxi` は 12件が 12件とも本人。`site:x.com from:youyuxi` は
...
test("x の結果には読み方を添える(要約を捨てさせない)", () => {
  // 実測(2026-08-11、端から端まで4回目): この一行が無かった回、外を見る役は
...
test("web に site:x.com と書かれたら x の先で受ける", async () => {
  // 実測(2026-08-11、端から端まで2回): 道具の説明に `where: ["x"]` と書いた後でも、
...
test("x — 投稿でない項は落とす(site:x.com を守らない索引が混ざる)", () => {
  // 実測(2026-08-11): 6問 268 件のうち 113 件が投稿でなかった。**混ざっていた実物**を並べる。
  const hits = parse("x", {
    results: [
...
test("x — 殻を拾った要約は外す。ただし題に中身が残っているなら項は残す", () => {
  // 実測(2026-08-11): 8問 162 件のうち 28 件(17%)の要約がログイン前の画面の文言だった。
