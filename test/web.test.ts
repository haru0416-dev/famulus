...
test("main も article も無い頁は class/id で本文を選ぶ", () => {
  // 実測 2026-08-10、実サイト16件: はてブ・価格.com はどちらも `<main>`/`<article>` を1つも
...
test("本文だけ切り出しても、どの頁かは残す", () => {
  // 実測 2026-08-10: 塊を選ぶようにしたら Wikipedia 日本語版から「日本語 - Wikipedia」が、
...
test("切れた script を本文として渡さない", () => {
  // 実測 2026-08-10、YouTube の視聴頁: 400KB で切ると `<script>` の開き13に対し閉じ12。
...
test("本文が組み上がらない頁でも、meta の題と説明は返す", () => {
  // 実測 2026-08-10: YouTube・ニコニコ・note・Bluesky・Mastodon は本文が 0〜50字しか無い。
...
test("空白だけの行で枠を食わない(CR・全角空白・ゼロ幅も空白として畳む)", () => {
  // 実測 2026-08-10、実サイト8件: 返した本文に占める空白行の割合は
...
test("属性の中の `>` をタグの終わりと取り違えない", () => {
  // 実測 2026-08-10、en.wikipedia の TypeScript: infobox が wikitext を丸ごと属性に持っていて、
...
test("実体参照は名前でも数値でも戻す", () => {
  // 実測 2026-08-10、実サイト14件の窓12,000字に生で残っていたもの:
...
test("新着一覧から書き手を落とさない", () => {
  // 実測 2026-08-11: Zenn のトピック feed は1件ごとに dc:creator を持っているのに拾っていなかった。
...
test("続きを読むのに取り直さない(全文を覚えて切り出す)", async () => {
  // 実測 2026-08-10: 「effect の最新安定版と公開日」1問で外へ出た 10 回が全部
...
test("大きい頁は窓で舐めず、語で当てる", () => {
  // 実測 2026-08-10: 40万字の registry JSON を offset で 300,000→408,000 まで 12,000字刻みで
...
test("本文が頭 400KB より後ろにある頁も読む", async () => {
  // 実測 2026-08-10: JS で組み立てる頁は、人が読む文字を JS の後ろに置く。
...
test("読めている頁に「別を当たれ」と言わない", async () => {
  // 実測 2026-08-10(86件の巡回): 一段の判定だと tenki.jp 1,719字・Yahoo 天気 1,954字・
