 ...
 *
 * famulus-zero がここで正しかったのは「消せない台帳を SQL 側で強制した」こと。
 * 忘却は DELETE ではなく `forget` イベントの追記、抹消は `content := NULL` の UPDATE だけ
 * (それ以外の UPDATE はトリガが ABORT する)。移植でこの性質を薄めない。
 *
 * ここが famulus-zero の実体的な弱点でもあった: **events 5件**。器はあるが溜まっていない。
 * なので `remember` は「使う側が呼びやすい形」に寄せてある(引数を最小、既定値を厚く)。
 * 溜まらない原因が API の摩擦なら、それは設計で消せる。
 */
...
export class Memory extends Effect.Service<Memory>()("Memory", {
  effect: Effect.gen(function* () {
     ...
     *
     * 並びは **bm25 の関連度**。ここが `at DESC` だった頃は「一致した中の新着順」でしかなく、
     * 心拍が毎回書く長い自己言及が"新しい"というだけで上位を占め、探している事実を押し下げていた
     * (`recall 予約` の上位10件のうち5件が自分の独り言だった)。
     *
     ...
     * 実際に「最近疲れてる」と言われた 49 秒後に「それ昨日も言ってる、8/9 の夕方に同じ言葉が残ってる」と
     * 返した — 台帳にその記録は無く、当たっていたのは今の入力だった。
     * 自走側は `text: ""` で索引に入れないことで同じ穴を塞いでいるが、対話の入力は索引に要る
     * (溜まらないと育たない)ので、除外は検索の側でやる。
     */
    const recall = (query: string, limit = 10, exclude?: string) =>
      Effect.gen(function* () {
        // **空白は語の区切りとして扱う**。ここを1本のフレーズとして投げていた頃は
        // 「エージェント メモリ」が「該当なし」になっていた — 空白ごと含む行しか当たらないので。
        // 台帳が2桁のうちは気付かないが、絞り込みたくなるのは中身が増えてからで、
        // ちょうど役に立ち始めた時点で使えないことが分かる形になっていた。
