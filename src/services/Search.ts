 ...
 *
 * ## 一般の web は、鍵を買わずに自前で立てた(2026-08-11)
 *
 ...
 *
 * ### 普段遣いに合わせて 12 → 9 に削った(2026-08-11)
 *
 ...
 * Zenn へは 25 回叩いていた(2026-08-11 実測)。回数制限は返ってこなかったが、そういう量になる。
 *
 ...
 *
 * 実測(2026-08-11、端から端まで動かした回): モデルは web 検索の癖で `site:zenn.dev/articles "Effect"`
 ...
 *
 * Qiita の API は記事本文を丸ごと返すので、1件で 100KB を超えることがある。実測(2026-08-11):
 ...
 *
 * 実測(2026-08-11、`Environment API` を付けて SearXNG に投げた4通り):
 ...
 *
 * 端から端まで動かした回(2026-08-11)に、モデルは `site:x.com/youyuxi ...` と自分で書いていた。
 * ただし `web` の先へ投げていたので、`site:` を無視する索引の結果が混ざっていた。
 * 書き方は合っていたので、**その書き方をこちらで受ける。**
 */
/**
 * 索引が投稿ではなく**ログイン前の画面**を拾ったときに要約へ出る文言。実物から取った(2026-08-11)。
...
const SOURCES: readonly Source[] = [
  {
    ...
    // 実測(2026-08-11、索引を 9 に削ったあと): 6問で 26〜57 件、中央値 1.41 秒。
  ...
  },
  {
    ...
    // 実測(2026-08-11、端から端まで4回目): この注が無かった回、外を見る役は
    ...
    // 本文が取れる口も全部断られている(2026-08-11 実測):
    ...
    // SearXNG に問い合わせるだけ。実測(2026-08-11): `site:x.com vite environment api` で
    ...
    // その中に vite.dev・github.com・en.wikipedia.org・vite.js.cn が入っていた(2026-08-11 実測)。
  ...
  },
  {
    ...
    // 実測(2026-08-11)「AIエージェント 記憶 設計」で返った8件は、チャットボット以外の7件が
  ...
  },
  {
    ...
    wide: true,
    // **語は 100 文字まで。** 101 文字で 400 が返る(2026-08-11 実測、境目は 100/101 でちょうど。
  ...
  },
  {
    ...
    // (2026-08-11 実測)。読む上限 1.5MB を超えると JSON の途中で切れる。5 件に抑えたうえで、
    // それでも切れたときのために `partialArray` で閉じている項だけ拾う。
    //
    // **並びは新着順で、変えられない。** `sort=rel` を付けても返りは1文字も変わらなかった
    // (2026-08-11、同じ問いで完全一致)。そのままだと当日投稿の ♡0 ばかりが5件並ぶ。
    ...
    // 2回引くと1問で使い切る。実測(2026-08-11、Biome v2 の移行を訊いた端から端まで1回):
  ...
  },
  {
    ...
    // 実測(2026-08-11): `tags=show_hn` 全体で 11,327 件、`(story,show_hn)` で 480,546 件。
  ...
  },
  {
    ...
    // **口を媒体ごとに分けている。** `(site:a OR site:b)` は索引が潰す(実測 2026-08-11:
    ...
    //
    // 募集頁だけにしてから測り直した(2026-08-11、3問×3媒体×上位10件、
    ...
    parse: (b) => {
      ...
      // (2026-08-11、4問×3媒体、募集頁175件を開いて受付中か見た): 新しい順の上半分 64/88、
      ...
      for (const h of searxngHits(b)) {
        ...
        if (!m) continue
        // **題が URL のままの項は落とす。**索引が題を取れていない頁で、実測(2026-08-11、
 ...
 * (2026-08-11、560秒/847秒)、`job` に届いた語は全部この形だった —
 ...
 * 実測(2026-08-11): marginalia は速い日で 0.8 秒、返らない日は無反応のまま。
 ...
 *
 * 実測(2026-08-11): 端から端まで1回動かしただけで Qiita の 1問 15 回を使い、無認証の枠
 ...
 * (2026-08-11 実測: 7先を同時に出して 0.9秒。順に出すと合計 4.5秒だった)。
 */
export async function searchWeb(query: string, opts: SearchOptions = {}): Promise<readonly SourceResult[]> {
  ...
  //
  // **これは道具の説明で直せなかった。** 端から端まで2回動かした実測(2026-08-11):
