 ...
 *
 * **X は、この線引きで全部落ちた。** 本文の取れる口を4つ測って、4つとも断られている:
 ...
 *   桁が違う。測り直したら、25 検索の1問で **46 リクエスト**を出して
...
const SOURCES: readonly Source[] = [
  ...
  },
  {
    ...
    // 語がほとんど効かなくなる — 「rust」で返った上位4件が全部その日の投稿で、
  ...
  },
  {
    ...
    reading:
      "**募集の終わったものが混ざる。**索引を読んでいるため — 実測(4問×3媒体、" +
    ...
    // **口を媒体ごとに分けている。** `(site:a OR site:b)` は索引が潰す(実測: 2媒体の OR で
    // 26件中 6件、1媒体ずつなら 30件中 20件)。`Source.url` が配列を返せるのはこのため。
    //
    // 媒体を3つに絞ったのは、`site:` で引いて個別の募集頁が索引にあるものだけ残したから。
    // **落とした先**: Findy Freelance(30件中0件 — 案件は login の内側)、
    // ココナラ(22件中2件、しかも分類頁)、Offers(34件中1件、残りは記事)。
    //
    // **期間で絞らない。撤回した判断。** 最初は `time_range=week` を付けて「受付中と語の当たりを
    // 両方満たす件数で週が勝つ」と書いていたが、**あれは `JOB_PATHS` で募集頁に絞る前の生の結果を
    // 数えていた** — react.dev や媒体の索引頁が「期限切れでない」側に入っていた。募集頁だけで
    // 測り直すと(3問×3媒体×上位10、**募集頁であり・語が当たり・開いたら受付中**)
    // 週 2件・月 11件・年 0件に対し、**絞りなし 23件**。週で絞ると
    // 「React フロントエンド 週1 リモート」の上位10件が経理サポートや OCR 校正になる。
    // 絞りを外したぶん `broaden` も消えて、SearXNG を叩く回数が半分になった。
    wide: false,
    ownOrigin: searxngBase,
    unavailable: () =>
      searxngBase() ? undefined : "OPEN_ZERO_SEARXNG が空にされている(SearXNG を使わない設定)",
    ...
    parse: (b) => {
      ...
      // **期間で絞るのをやめたので、鮮度を保つのはこの並べ替えだけ。**効きは `reading` の数字どおりで、
      // 上に出ても4件に1件は終わっている。それを承知で「開いて確かめる」と言わせている。
