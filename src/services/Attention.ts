 ...
 *
 * 対話だけで動く間は、次に何をするかは常に持ち主の発話が決めていた。自走に切り替えると
 * その入力が無い時間のほうが長くなるので、「起きたとき何を見るか」を台帳に置いておく必要がある。
 * schema.sql には最初から `watchlist`(見張っている未決事項)と `questions`(未 probe の仮説)が
 * 用意されていて、どちらも使われていなかった。自走にして初めて意味を持つ2枚なので、ここで繋ぐ。
 *
 * `questions` を `belief_slots` と分けてあるのが効いてくる。自走中は答え合わせをしてくれる相手が
 * いないので、**推測を belief に昇格させない置き場**が無いと、確認していないことが事実として溜まる。
 *
