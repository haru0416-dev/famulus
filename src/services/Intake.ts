 ...
 *
 * open-zero の記憶が空だったのは設計が悪かったからではなく、入口が無かったから。
 * 持ち主の言葉は二箇所に溜まっていて、どちらもここからは見えていなかった。
 *
 * ── 入口1: Claude Code の作業ログ(`~/.claude/projects` の JSONL)
 *
 *   生ログ合計    553.5MB   81 セッション・36 日
 *     人の発話      187KB  0.03%  1761 件  ← 1文字も削らない
 *     応答の地の文  7.0MB   1.3%           ← 1往復あたり1件に畳む
 *     道具の入出力  546MB  98.7%           ← 全部捨てる
 *
 * ── 入口2: Claude.ai の書き出し(`.data/claude-export`)
 *
 *   会話 612 件ぶんの見出しが入っているが、**本文が残っているのは 73 件だけ**。
 *   残り 539 件は uuid と日時だけの殻で、text も content も空になっている
 *   (書き出し側の都合。こちらでは復元できない)。手が届くのは 2026-05-28 以降。
 *
 *     conversations.json   本文のある会話 68 件 / 人の発話 455 件・60KB
 *     design_chats/        37 件(30 件に人の発話)
 *     memories.json        **欠けていない。** Claude.ai 側が半年かけて作った持ち主の像
 *
 * **人の言葉はどちらの入口でも数パーセントに満たない。** だから圧縮は「小さくする工夫」ではなく、
 * *何が人の判断で、何が作業の残骸か* を分けることそのものになる。比率を削るのではなく境界を引く。
 *
 * 2段階に分けてある。
 *   1. 選別(モデルを使わない) … 道具の入出力を捨て、人の発話を無傷で残し、応答を畳む。
 *      ここで 500 倍落ちる(553MB → 1.1MB)。**枠を1回も使わずに効果を測れる**のが要点。
...
/**
 * 読んだ JSON を持っておく。`conversations.json` は 17MB あって、
 * 走査と取り込みで何度も開く。**mtime が変わったら捨てる**ので、差分の zip を足しても効く。
 */
 ...
 * 546MB のうち大半がこれで、しかも「何を決めたか」は一度も書かれていない。
 */
...
function readSession(path: string): { ref: SessionRef; turns: Turn[]; rawBytes: number } | undefined {
  ...
  // 全文パースは高いので、人が打った跡が無いファイルはここで捨てる(2922 本中 81 本しか残らない)。
 ...
 * `material` は取り込みのたびに呼ばれるので、毎回 553MB を舐め直すと入口が使い物にならなくなる。
 */
...
/**
 * 素材にする。**持ち主の発話は1文字も削らない**(平均 52 文字。削る意味がないし、削ると原文が消える)。
 *
...
function compress(turns: readonly Turn[]): string {
  ...
  // 全部を頭 100 文字ずつ残すより、答えの1件を厚く残すほうが同じ量で情報が多い。
  const folded: Turn[] = []
 ...
 *
 * **どの項目にも `said`(持ち主の発話からの引用)を必須にしてある。** これが要点。
 * `decisions` にだけ引用を求めていた版では、素材の量で勝る応答側の地の文に引きずられて
 * 「REST API が完全動作することを確認」のような**作業報告**を「決定」として書いてきた。
 * それは誰の判断でもない。引用を要求すると、持ち主の言葉に根拠が無い項目は書けなくなる。
 *
 * 引用を `preferences` と `corrections` にも広げたのは、**そこが一番効く層だから**。
 * 「持ち主はこういうやり方を好む」は本人像そのもので、しかも自由記述だと
 * モデルが読み取った印象と本人が言ったことの区別が付かない。台帳に入った後では
 * どちらだったかを誰も復元できない — 代わりを務めるなら、そこは常に本人の言葉に戻せること。
 */
const DIGEST_SCHEMA = {
  type: "object",
  additionalProperties: false,
...
export class Intake extends Effect.Service<Intake>()("Intake", {
  effect: Effect.gen(function* () {
     ...
     *
     * これは会話ログではなく、Claude.ai 側が半年かけて作った持ち主の像そのもの。
     * トピック別の記憶ファイル16本と、散文の要約4節が入っている。
