 ...
 *
 * 台帳には2つの層がある。`import`(過去の会話から起こした要約)は「その日時点でそう書かれていた」
 * という記録でしかなく、今の事実として使ってよいのは `belief` で確定した値だけ — recall の
 * 読み方がそう書いてある。ところが確定へ上げる口は道具(`remember` の `slot`)しか無く、
 * **持ち主が話した回にモデルがそれを呼ぶかどうかに全部かかっていた。** 呼ばれないまま流れると、
 * 台帳は増えるのに引ける値は増えない。実際にそうなっていた(docs/adr/0014)。
 *
 ...
 *
 * **材料は持ち主の入力だけ。** 外から来たもの(web / gmail)は確定に上げない。
 * 確定値は「今の事実」として後の回に無検査で使われるので、ここを外に開くと、
 * 拾ってきた文が事実として台帳に入る道ができる。引用がコード側で照合できるのも、
 * 材料を持ち主の入力に限っているから成り立つ。
 */
...
export const KEEPER_SYSTEM = `あなたは台帳の記録係です。持ち主とのやり取りが1回終わったところで、
**そこで確かめられた値**を確定へ上げるかどうかだけを決めます。文章は書きません。

確定に上げてよいのは、**持ち主が言ったこと**に限ります。以下は上げません:
- あなた(または過去のあなた)の推測・要約・言い換え
- 外から拾ってきたもの(web・メールの中身)
- 「〜かもしれない」「〜のはず」— 持ち主が確言していないもの
- 一度きりの出来事の実況(「いま出かける」)。**次に引いて意味を持つ値**だけを上げます

上げるものには、持ち主の発言からそのまま写した一節を付けます。**写せないなら上げません。**
    ...
    values: {
      ...
      items: {
        ...
        properties: {
          ...
          quote: {
            type: "string",
            description: "根拠。持ち主の発言からそのまま写した一節。要約や言い換えにしない。",
          },
          validFrom: {
            type: "string",
 ...
 *
 * 「持ち主が言ったことだけ」は書いておけば守られる類の制約ではない。守られなかったときに
...
/**
 * 締めの記録係を1回通す。**失敗しても回そのものは落とさない。**
 *
 * 戻り値は台帳に残す1行。呼び出し側はこれを心拍の記録に添えるだけで、経路の分岐には使わない
 * — 記録係が転んだせいで返信や既読位置が変わると、直す場所が分からなくなる。
 */
export const keep = (opts: { material: string; since?: string; signal?: AbortSignal }) =>
  Effect.gen(function* () {
    const runner = yield* Runner
    const mem = yield* Memory
    if (bare(opts.material).length === 0) return "記録係: 材料が無い(持ち主の発言がこの回に無い)"

    // 既存の slot を見せる。**別名を作らせないため** — 同じ事柄が2つの名前で入ると、
    // どちらを引いても片方しか出てこない台帳になる。
    ...
    // **この回で既に確定した slot には触らない。** 記録係は取りこぼしを拾う網であって、
    ...
    if (out._tag === "Left") return `記録係: 呼べなかった(${causeReason(out.left)})`
    ...
    const head = kept.length === 0 ? "記録係: 上げるものは無かった" : `記録係: ${kept.length} 件を確定へ`
