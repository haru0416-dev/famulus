 ...
 *
 * keeper(docs/adr/0014)は回の締めに必ず通るが、材料はその回のユーザーの発言だけ。
 * 「3日にわたって同じことを言っている」「先週の値が今週の発言で変わった」の類は、
 * 1回ぶんの材料の中に現れないので上がらない。ここは窓を日単位に広げて同じ判定を通す。
...
/** 1回で読む発言の上限。**入力の量をここで決める。** 超えた分は次の回に残る(取りこぼさない)。 */
 ...
 *
 * 前日ぶんの発言が出揃っていて、かつユーザーが枠を使っていない時間帯に置く。
...
/**
 * 窓を広げたぶんだけ、判定に足すもの。**keeper の本文は書き換えない** —
...
- **繰り返し出てくるもの。** 同じ依頼・同じ関心が**別々の機会に**何度も出ているなら、
  それは一度きりの出来事ではありません。次に引いて意味を持つ値として上げてよいものです。
...
- **値が変わったもの。** 前に言われた値と食い違う発言が後の日にあるなら、
  同じ slot に新しい値を上げます(区間が継がれます)。\`reason\` に何がどう変わったかを書きます。
  こちらは \`interest.\` ではなく、既存の slot の名前をそのまま使います。

回数を数えたこと自体は根拠になりません。**上げるものには必ず、写せる一節が要ります。**`
...
/**
 * 材料を集める。**モデルを呼ばない。** 数えるだけで効き目が見られるようにしてある。
 *
 ...
 *
 * `dry` は材料を数えるだけで返る。**枠を使わずに「今夜あげるものがあるか」が分かる**ので、
 * 動きを確かめるときはこちら。
 */
export const dream = (opts?: {
  ...
  Effect.gen(function* () {
    ...
    const line = yield* keep({
      material,
      // 窓の始まりを渡す。**この窓の中で既に確定した slot は触らない** —
