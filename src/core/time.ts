 ...
 * 日付境界。**持ち主の1日**で数える。
 *
 * famulus-zero は日次/月次の集計を `substr(at,1,10)`(= UTC の日)で切っていた。
 * 記録そのものは UTC で持つのが正しいが、**上限が切り替わる瞬間を UTC にすると
 * 日本時間の朝9時に日次枠がリセットされる**。「今日はあと何回使えるか」を人が判断する量なので、
 * 境界は人の1日に合わせる。実際、対話中にエージェント自身が「budget が返した日が今日ではない」と
 * 指摘してきたのがこの実装を入れた理由。
 *
 ...
 *
 * **これが無いと、夜中の記録が前日として読まれる。** 実測(2026-08-11 00:52 JST): 台帳は
 * `2026-08-10T15:52Z` で持ち、`renderRecall` はそれを「2026-08-10 15:52」と帯なしで渡していた。
 * モデルには別口で「今日は 2026年8月11日」と入るので、**51分前の出来事を昨日の午後として読む**。
 * 日本時間の 0〜9時はまるごとこの穴に落ちる。日付境界(dayRange)は既にローカルで数えているので、
 * ずれていたのは見せ方だけ。
 *
