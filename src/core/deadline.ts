 ...
 *
 * tick は `OPEN_ZERO_TICK_TIMEOUT_MS`(現在の既定は7分)で切られる。切られるとその回は丸ごと落ちて、
 ...
 *
 * process 単位の可変にしてあるのは、1プロセスで同時に走る tick が1つだけで、
...
/** 締切を外す。対話(`bun run agent` / `oz`)は時間で切られないので、既定はこちら。 */
