 ...
 *
 * 心拍は 5 分で切られる(`OPEN_ZERO_TICK_TIMEOUT_MS`)。切られるとその回は丸ごと落ちて、
 * `commit` に辿り着かないので既読位置も冷却の起点も進まない。**次のタイマーが同じ理由で
 * また起きて、また 5 分使って、また落ちる。** 冷却は「実際に動いた」印を起点に数えるので、
 * 落ち続けるかぎり後退が一度も掛からない — 15 分ごとに 5 分ずつ焼き続ける輪になる。
 *
 * 実測(2026-08-12 17:14 JST): 走らせる口を足した最初の心拍が、拾ったものを 9 回走らせた末に
 * 300 秒で切られた。走行そのものは各 0.7〜7.9 秒で、記録も9件残っていた。足りなかったのは
 * **止めどきの判断**で、モデル側には残り時間を知る手立てが無かった。
 *
 * だから時計を渡す。プロンプトに「数分で切られる」と書くだけでは足りない —
 * 書いてあるのは起動時の話で、9 回目を走らせるかどうかを決める時点では既に過去の文になっている。
 *
 * process 単位の可変にしてあるのは、心拍が1回の起動につき1つの締切しか持たないから。
 * 道具は Flue のフックの中から呼ばれ、心拍の呼び出し文脈を引数で受け取れない。
 */

/** 締切(epoch ms)。未設定なら締切なし = 対話。 */
let deadlineAtMs: number | undefined

/** この回の持ち時間を宣言する。心拍が dispatch の直前に1回だけ呼ぶ。 */
