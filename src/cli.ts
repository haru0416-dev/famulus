 ...
 *   oz intake [--dry] [n]  … 過去の会話を圧縮して台帳に入れる(台帳の入口)
 *
...
const USAGE = `oz — open-zero の裁可 CLI
  ...
  oz intake [n]            未取り込みの会話を古い順に n 件(既定 10)台帳へ入れる
  ...
  Effect.gen(function* () {
    ...
    switch (cmd) {
      case "status": {
        ...
        // **枠は2つある。** 声は claude-max、作業と調査は chatgpt-rmod。片方だけ見ていると
        ...
        // ここを出さないと「静かなのは用が無いからか、何も知らないからか」が持ち主に分からない。
        ...
        return [
          ...
          // 心拍は黙って死ぬ。**最後に呼ばれた時刻**を出しておかないと、
          // 「静かなのは用が無いからか、止まっているからか」が持ち主に区別できない。
          last
            ? `心拍: 最終 ${last}(最後に実際に動いたのは ${lastActive ?? "まだ無い"})`
      ...
      case "attention": {
        ...
        return [
          `いま ${d.at}`,
          d.idle
            ? `次の心拍は動かない(冷却 ${d.cooldownHours} 時間 / 前回の実働から ${Number.isFinite(d.sinceLastActiveHours) ? `${d.sinceLastActiveHours.toFixed(1)} 時間` : "まだ無い"})`
            : `次の心拍は動く: ${d.reasons.join(" / ")}`,
          "",
          `見張り(${watches.length} 件)`,
          ...(watches.length === 0
            ? ["  なし"]
            : watches.map(
                (w) =>
      ...
      case "list": {
        ...
        if (rows.length === 0) return status === "proposed" ? "裁可待ちは無い" : "該当なし"
      ...
      case "approve": {
        ...
        return [
          ...
          "",
          "注意: **まだ実行はされていない**。コネクタが無いので実行器は動いていない。",
          "      状態は「裁可済み・未実行」で止まっている。",
        ].join("\n")
      }
      ...
      case "belief": {
        ...
        return [
          ...
          `  ${localStamp(now.validFrom)} から(台帳が知ったのは ${localStamp(now.updatedAt, false)})`,
          "",
          `変遷(${hist.length} 件)`,
